...
ecoregions <- 
  terra::vect("data/raw_data/ecoregions/ecoregions.shp") |> 
  terra::project(x = _, y = bam_boundary) |>
  terra::crop(x = _, y = bam_boundary) 

# identify ecoregions within bam_template
...
ecoregions_subset <- ecoregions[which(keep)]
plot(ecoregions_subset)
identical(ecoregions, ecoregions_subset)
# [1] TRUE -- no regions were dropped

# save 
terra::writeVector(ecoregions_subset, file.path(ia_dir, "ecoregions_masked.gpkg"), overwrite=TRUE)
