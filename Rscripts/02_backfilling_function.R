...
neworder <- c("Peatland_1km", "SCANFIclosure_1km", "SCANFIbiomass_1km", "SCANFIheight_1km", "SCANFIheightcv_1km", 
...
# is every biotic variable in both `neworder` and `df_bcr14_2020`?
any(!(biotic_vars_thinned %in% neworder)) # FALSE
any(!(neworder %in% biotic_vars_thinned)) # FALSE

# re-order biotic variable hierarchy
...
df <- data.frame(covariate=character(), range_lo=numeric(), range_hi=numeric(), index=numeric())
for (v in 1:length(biotic_vars_thinned)) {
  if (biotic_vars_thinned[v] %in% colnames(df_bcr14_2020)) {
    range_lo <- range(df_bcr14_2020[,biotic_vars_thinned[v]], na.rm=T)[1]
    range_hi <- range(df_bcr14_2020[,biotic_vars_thinned[v]], na.rm=T)[2]
    covariate <- biotic_vars_thinned[v]
    df[v,] <- data.frame(covariate=covariate, range_lo=range_lo, range_hi=range_hi, index=v)
  } else NULL
}
...
rm(nice_var_names, biotic_vars, range_lo, range_hi, v, covariate, df); gc()
...
backfill_landscape <- function(i){
  ...
  # log transform covariates to prevent negative values in predictions
  # (see above: ranges of each covariate were checked)
  ...
  # add newly predicted biotic feature as a predictor for the next iteration
  predictor_vars <- c(predictor_vars, paste0(biotic_vars_thinned[i],"_bf"))
  predictions_df <-  tibble(!!paste0(biotic_vars_thinned[i],"_bf") := predictions_i, lon = CanHF_1km_present$lon, lat = CanHF_1km_present$lat)
  df_bcr14_2020 <- dplyr::left_join(df_bcr14_2020, predictions_df, by=c("lon", "lat"))
  ...
  # set water bodies to zero (to match BAM 5.0 covariate stack format)
  predictions_raster_i <- terra::cover(predictions_raster_i, stack_bcr14_2020[biotic_vars_thinned[i]])
...
backfilled_raster_stack <- do.call(c, backfilled_rasters)
names(backfilled_raster_stack) <- biotic_vars_thinned

terra::writeRaster(backfilled_raster_stack, file="C:/Users/mannf/Proton Drive/mannfredboehm/My files/Drive/boreal_avian_modelling_project/ImpactAssessment/data/derived_data/backfilled_rasters/BCR14_backfilled_continuous.tif")
